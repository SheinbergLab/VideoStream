...
class DservSocket
{
  ...
  int listening_socket_fd;  // Store for shutdown
...
public:
  ...
  void set_queue(SharedQueue<std::string> *queue) { ds_queue = queue; }
  
  void shutdown(void)
  {
    m_bDone = true;
    
    // Close listening socket to wake up accept()
    if (listening_socket_fd >= 0) {
#ifndef _MSC_VER
      ::shutdown(listening_socket_fd, SHUT_RDWR);
      close(listening_socket_fd);
#else
      ::shutdown(listening_socket_fd, SD_BOTH);
  ...
  start_ds_tcp_server(void)
  {
	...
	continue;
      ...
      #endif
      // CHANGED: Pass 'this' pointer to the static method
      std::thread thr(ds_client_process, this, std::move(new_socket_fd));
      thr.detach();
    }
